...
export default function Webmaster(){
 ...
 [perm,setPerm]=useState({}),[allPerm,setAllPerm]=useState([]),[playlists,setPlaylists]=useState([]),
 ...
 async function load(){
  ...
  try{
   ...
   setPlaylists(rj.playlists||[]);setSchedules(rj.schedules||[]);setSite(oj.settings||site);
 ...
 return <main className="admin">
  ...
  <div className="adminWrap">
   ...
   <div className="workspaceTabs">{[
    ['overview','📊 Genel'],['team','👥 Kullanıcılar'],['permissions','🔐 Roller'],['radio','📻 Radyo Motoru'],
   ...
   {tab==='radio'&&<><section className="panel"><div className="panelTitle"><h2>📻 101dunyasi Radio Engine</h2><span>{data?.radio?.live?'🔴 DJ CANLI':'🎵 OTOMATİK'}</span></div><div className="liveBox"><b>{data?.radio?.live?`🎙️ DJ ${data?.radio?.dj||''}`:'🎵 Otomatik yayın'}</b><span>{data?.radio?.title||'101dunyasi.com'}</span><small>{data?.radio?.currentTrack?.name||'Arşiv parçaları ffmpeg ile ortak stream üzerinden yayınlanır.'}</small></div><div className="settingsGrid"><label>Otomatik fallback<input type="checkbox" checked={!!site.autoFallback} onChange={e=>setSite({...site,autoFallback:e.target.checked})}/></label><label>Fallback bekleme (sn)<input type="number" min="1" max="60" value={site.fallbackDelaySec||5} onChange={e=>setSite({...site,fallbackDelaySec:Number(e.target.value)})}/></label><label>Yavaş mod (sn)<input type="number" min="0" max="30" value={site.slowModeSec||0} onChange={e=>setSite({...site,slowModeSec:Number(e.target.value)})}/></label></div><button className="big" onClick={async()=>{const r=await fetch('/api/webmaster/radio',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'settings',autoFallback:site.autoFallback,fallbackDelaySec:site.fallbackDelaySec,slowModeSec:site.slowModeSec,badWords:site.badWords||''})});setMsg(r.ok?'Radyo motoru ayarları kaydedildi.':(await r.json()).error);load()}}>💾 Radyo Ayarlarını Kaydet</button></section><section className="panel"><h2>✅ Kendi radyo zincirimiz</h2><div className="proUserGrid"><div className="proUser"><span className="avatar">🎙️</span><div><b>DJ Server Stream</b><small>Ortak sunucu canlı sesi</small></div></div><div className="proUser"><span className="avatar">🎵</span><div><b>Auto Radio</b><small>/radio/live.mp3 ortak yayın</small></div></div></div></section></>}
