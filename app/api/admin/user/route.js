...
export async function DELETE(req){
  ...
  const data=readData();const target=data.users.find(u=>String(u.id)===String(id));
  if(!target)return NextResponse.json({error:'Kullanıcı bulunamadı.'},{status:404});
  if(actor.role==='ADMIN'&&roleLevel(target.role)>=4)return NextResponse.json({error:'Bu hesap Admin yetkisinin üstünde.'},{status:403});
  data.users=data.users.filter(u=>String(u.id)!==String(id));audit(data,actor.username,'user_delete',target.username);writeData(data);return NextResponse.json({ok:true});
}
